...
# Capa de unidades compartida con el pipeline de análisis (analysis/common/units.py).
...
def process_sqlite(db_path, num_gpus, out_dir):    
    ...
    # Encabezados con unidad explícita: tiempos/latencias en µs, tamaños en bytes
    headers = (['GPU_ID', 'kernel_time_us']
               + [m + ('_bytes' if m.endswith('_size') else '') for m in METRICS_TO_SUM]
               + [m + '_us' for m in METRICS_TO_AVG])
    ...
    with open(raw_csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for gpu_id, metrics in sorted(gpu_data.items()):
            # kernel_time llega en segundos, se exporta en µs
            row = [gpu_id, metrics['kernel_time'] * units.S_TO_US]
            ...
            for m in METRICS_TO_AVG:
                ...
                # latencias llegan en segundos, se exportan en µs
                row.append(avg_val * units.S_TO_US)
                if len(metrics[m]) > 0:
    ...
    run_summary['mem_throughput_gbps'] = units.throughput_gbps(total_bytes, sim_time_sec)
    run_summary['compute_throughput_ginstps'] = (total_inst / (10**9)) / sim_time_sec if sim_time_sec > 0 else 0.0
    run_summary['max_kernel_time_us'] = sim_time_sec * units.S_TO_US
...
def main():
    ...
    if summaries:
        ...
        headers = [
            ...
            'Global_Avg_Read_Latency_us', 'Global_Avg_Req_Latency_us', 'Global_Avg_Write_Latency_us'
        ]
        ...
        with open(summary_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for s in summaries:
                row = [
                    ...
                    f"{s['avg_read_latency'] * units.S_TO_US:.6f}".replace('.', ','),
                    f"{s['avg_req_latency'] * units.S_TO_US:.6f}".replace('.', ','),
                    f"{s['avg_write_latency'] * units.S_TO_US:.6f}".replace('.', ',')
                ]
                writer.writerow(row)
